...
class SuperadminController extends Controller
{
    ...
    {
        return view('auth.admin.pages.staff.staff', [
            'user_data' => Auth::user(),
    ...
    /*** Update Staff ***/
    public function hs_supdatedstore(Request $request, $id)
    {
        $validated = $request->validate([
            'name'    => 'string|max:255',
            'email'   => 'email',
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('superadmin.staff')->with('success', 'User updated successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->route('superadmin.staffcreate')->with('error', 'Failed to update user: ' . $e->getMessage());
        }
    }
